  ...
  keys.forEach((key, keyIndex) => {
    if (keyIndex < keys.length - 1) {
      ...
      if (next instanceof Array && !nextKey.match(/\d/)) {
        console.error(`this state will not run correctly, you are setting Attribute [${nextKey}] into an Array`)
      }
    } else {
      if (next instanceof Array && !key.match(/\d/)) {
        console.error(`this state will not run correctly, you are setting Attribute [${key}] into an Array`)
      }
...
export const getDerivedState = function(state, node) {
  ...
  if (node.dataSource && node.dataSource.state) createState(state, node.dataSource.state);
